...
class BaseSegmentor(nn.Module):
    ...
    def forward_test(self, imgs, img_metas, **kwargs):
        ...
        # for var, name in [(imgs, 'imgs')]:#不檢查img_metas
        #     if not isinstance(var, list):
        #         raise TypeError(f'{name} must be a list, but got '
        #                         f'{type(var)}')
        
        num_augs = len(imgs) if isinstance(imgs, list) else imgs.shape[0]
        ...
        if isinstance(imgs, list):
            if num_augs == 1:
                if img_metas==None:#防止出錯
                    return self.simple_test(imgs[0], None, **kwargs)
    ...
    def my_show_inference_result(self,
                    img,
                    result,
                    palette=None,
                    win_name='',
    ...
    def show_result(self,
        ...
        assert len(self.CLASSES) in [19, 150, 171, 2]
        ...
        pred = color_seg.copy()
        color_seg = img * 0.2 + color_seg * 0.8
        img = img.astype(np.uint8)
        ...
        elif len(self.CLASSES) == 2: #river
            gt_file = out_file[1].replace('/originalFrames/', '/MASK/')
            gt_file = gt_file.replace('.jpg', '.png')
#         elif len(self.CLASSES) == 2: #road
#             gt_file = out_file[1].replace('/RTK_SemanticSegmentationGT_originalFrames/', '/RTK_OnlyROAD_MASK/')
#             gt_file = gt_file.replace('.jpg', '.png')
